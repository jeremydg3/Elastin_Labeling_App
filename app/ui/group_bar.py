...
from PyQt6.QtWidgets import QLabel, QFrame, QHBoxLayout, QWidget
...
class GroupChip(QFrame):
    ...
    def _update_swatch(self, grey=False):
        c = QColor(GROUP_COLORS[self.gid])
        if grey:
            c = QColor(100, 100, 100)  # Darker grey for dark theme
        pm = QPixmap(self.swatch.width(), self.swatch.height())
        pm.fill(c)
    ...
    def set_active(self, yes: bool):
        self._active = yes
        if yes:
            self.setStyleSheet("QFrame { background-color: #2d2d2d; border: 2px solid #4CAF50; border-radius: 6px; }")
            self._update_swatch(grey=False)
        else:
            self.setStyleSheet("QFrame { background-color: #2d2d2d; border: 1px solid #4a4a4a; border-radius: 6px; }")
...
class EraserChip(QFrame):
    ...
    def _paint_badge(self, active: bool):
        # orange when active, dark grey when off (for dark theme)
        c = QColor(255, 170, 0) if active else QColor(100, 100, 100)
        pm = QPixmap(self.badge.width(), self.badge.height()); pm.fill(c)
        self.badge.setPixmap(pm)

    def set_active(self, yes: bool):
        self._active = yes
        self.setStyleSheet("QFrame { background-color: #2d2d2d; border: %dpx solid %s; border-radius: 6px; }" %
                           (2 if yes else 1, "#ff9800" if yes else "#4a4a4a"))
...
class GroupBar(QWidget):
    activeChanged = pyqtSignal(int)   # emits -1 when none
    eraserChanged = pyqtSignal(bool)    # True when eraser ON
